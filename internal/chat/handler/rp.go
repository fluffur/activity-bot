...
func (h *Handler) ListRPCommands(ctx *command.Context, u *ext.Update) error {
	...
	eb.Plain("• ")
	eb.Code("{speech}")
	eb.Plain(" — алиас для {detail}\n")
	eb.Plain("• ")
	eb.Code("{A}")
	eb.Plain(" / ")
	eb.Code("{B}")
	eb.Plain(" — короткие алиасы для {actor}/{target}\n")
	eb.Plain("• ")
	eb.Code("обнял(а)")
	eb.Plain(" — короткая форма для варианта по полу автора\n")
	eb.Plain("• ")
	eb.Code("{actor:male|female}")
	eb.Plain(" — вариант по полу автора\n")
	eb.Plain("• ")
	eb.Code("{target:male|female}")
