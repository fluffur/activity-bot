...
func (h *Handler) ShowNorm(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if err != nil {
		helpers.AnswerMessage(ctx, b, update, "Не удалось отправить норму чата")
		log.Println("Failed to show chat norm", err)
		return
	}

	helpers.AnswerMessage(ctx, b, update, fmt.Sprintf("Норма чата: %d сообщений", norm))
}
...
func (h *Handler) SetNorm(ctx context.Context, b *bot.Bot, update *models.Update) {
	if !helpers.CheckOwnerOrAdmin(ctx, b, h.adminService, update.Message.Chat.ID, update.Message.From.ID) {
		helpers.AnswerMessage(ctx, b, update, "Команда установки нормы доступна только создателю чата и администраторам бота")
		return
	}
	...
	if err != nil {
		helpers.AnswerMessage(ctx, b, update, "Норма должна быть числом")
		return
	}

	if err := h.service.SetNorm(ctx, update.Message.Chat.ID, norm); err != nil {
		helpers.AnswerMessage(ctx, b, update, "Не удалось установить норму чата")
		log.Println("Failed to set chat norm", err)
		return
	}

	helpers.AnswerMessage(ctx, b, update, "Новая норма чата установлена")
}
