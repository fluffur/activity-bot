...
func (r *ChatRepository) SetBotDeletedAt(ctx context.Context, chatID int64, time time.Time) error {
	return r.queries.SetChatDeletedBotAt(ctx, db.SetChatDeletedBotAtParams{
		BotRemovedAt: pgtype.Timestamptz{
			Time:  time,
			Valid: !time.IsZero(),
		},
		ID: chatID,
