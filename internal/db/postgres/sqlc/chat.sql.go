...
WITH ins AS (
    INSERT INTO chats (id, title)
        ...
        RETURNING id, norm_warn, newbie_threshold_days, ai_system_prompt, max_ladder, call_on_join, welcome_call_message, week_start_day, max_warns, norm_ban, command_prefix, allow_prefixless, mentions_per_message, mention_types, title, tags_enabled, week_start_time, broadcast_enabled, bot_removed_at)
SELECT id, norm_warn, newbie_threshold_days, ai_system_prompt, max_ladder, call_on_join, welcome_call_message, week_start_day, max_warns, norm_ban, command_prefix, allow_prefixless, mentions_per_message, mention_types, title, tags_enabled, week_start_time, broadcast_enabled, bot_removed_at
FROM ins
UNION ALL
SELECT id, norm_warn, newbie_threshold_days, ai_system_prompt, max_ladder, call_on_join, welcome_call_message, week_start_day, max_warns, norm_ban, command_prefix, allow_prefixless, mentions_per_message, mention_types, title, tags_enabled, week_start_time, broadcast_enabled, bot_removed_at
FROM chats
...
const setChatDeletedBotAt = `-- name: SetChatDeletedBotAt :exec
UPDATE chats
SET bot_removed_at = $1
WHERE id = $2
`

type SetChatDeletedBotAtParams struct {
	BotRemovedAt pgtype.Timestamptz `db:"bot_removed_at" json:"botRemovedAt"`
	ID           int64              `db:"id" json:"id"`
}

func (q *Queries) SetChatDeletedBotAt(ctx context.Context, arg SetChatDeletedBotAtParams) error {
	_, err := q.db.Exec(ctx, setChatDeletedBotAt, arg.BotRemovedAt, arg.ID)
	return err
}
