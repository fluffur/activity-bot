...
func (r *RestRepository) GetRestMembers(ctx context.Context, chatID int64) ([]chatmember.ChatMember, error) {
	rows, err := r.queries.GetRestMembers(ctx, chatID)
	if err != nil {
		return nil, err
	}

	return mapList(rows, func(row db.GetRestMembersRow) chatmember.ChatMember {
...
func (r *RestRepository) AddRequest(ctx context.Context, request rest.Request) error {
...
func (r *RestRepository) GetUserRestRequests(ctx context.Context, chatID, userID int64) ([]rest.Request, error) {
	...
	return mapList(rows, func(row db.GetUserRestRequestsRow) rest.Request {
		return mapRestRequestFull(row.RestRequest, row.ChatMember, row.User)
	}), nil
}
